...
class Solution
{
    public static void solve(Node curr, int sum, int curr_sum , ArrayList<Integer> path, ArrayList<ArrayList<Integer>> ans){
        if(curr == null) return;
        curr_sum += curr.data;
        path.add(curr.data);
        if(curr_sum == sum) ans.add(new ArrayList<>(path));
        if(curr.left != null){
            solve(curr.left, sum, curr_sum, path, ans);
        }
        if(curr.right != null){
            solve(curr.right, sum, curr_sum, path, ans);
        }
